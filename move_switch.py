...
def compare_port_snapshots(
    ...
    for serial in all_switches:
        ...
        all_ports = sorted(set(before_ports.keys()) | set(after_ports.keys()), key=lambda x: (len(x), x))
...
def move_and_validate_network(network: Dict, template: Dict, autobind: bool, dry_run: bool) -> Dict[str, Any]:
    ...
    after_snapshot = snapshot_switch_ports(network_id)
    diff = compare_port_snapshots(before_snapshot, after_snapshot)

    print(f"{bc.WARNING}Port Configuration Diff Report{bc.ENDC}")
    if diff["diff_rows"]:
        headers = ["Switch", "Port", "Setting", "Before", "After"]
        print(format_ascii_table(headers, diff["diff_rows"]))
    else:
        print(f"{bc.OKGREEN}No port configuration differences detected.{bc.ENDC}")

    print()
    print(
        f"{bc.OKBLUE}Port success rate:{bc.ENDC} {bc.OKGREEN}{diff['port_success_rate']:.2f}%{bc.ENDC} "
        f"({diff['unchanged_ports']}/{diff['total_ports_before']} unchanged ports)"
    )
    print(
        f"{bc.OKBLUE}Settings success rate:{bc.ENDC} {bc.OKGREEN}{diff['settings_success_rate']:.2f}%{bc.ENDC} "
        f"({diff['total_settings'] - diff['changed_settings']}/{diff['total_settings']} unchanged settings)"
    )
    if missing_switches:
        print(
